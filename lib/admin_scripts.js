...
jQuery(document).ready(function ($) {
	$("#continue_indexing").on("click", function () {
		$("#continue_indexing").addClass("relevanssi-is-loading").attr("disabled", "disabled");
		$("#build_index").attr("disabled", "disabled");
	...
	jQuery.ajax({
		...
		success: function (response) {
			...
			if (response.completed == "done") {
				jQuery("#continue_indexing").removeClass("relevanssi-is-loading").removeAttr("disabled");
				jQuery("#build_index").removeClass("relevanssi-is-loading").removeAttr("disabled");
...
jQuery(document).ready(function ($) {
	// Helper function to distribute the HTML payload to the main view and sidebar
	function renderSearchResults(response) {
		var $results = $("#results");

		$results.html(response);

		var $debugCard = $results.find("#debugging");
		var $sidebarTarget = $("#relevanssi-debug-sidebar-target");

		if ($debugCard.length && $sidebarTarget.length) {
			$sidebarTarget.html($debugCard);
		} else if ($sidebarTarget.length) {
			$sidebarTarget.empty();
		}
	}
...
jQuery(document).ready(function ($) {
	// Target all elements with the data-hover-target attribute
	$('[data-hover-target]').on('mouseenter', function () {
		// Find the target ID and add the class
		$('#' + $(this).data('hover-target')).addClass('active-highlight');
	}).on('mouseleave', function () {
		// Find the target ID and remove the class
		$('#' + $(this).data('hover-target')).removeClass('active-highlight');
	});
});
